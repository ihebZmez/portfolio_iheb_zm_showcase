...
const Experience = () => {
  ...
  // Map IDs to keys, assuming IDs 1 to 5 mapping to the keys in json
  // Let's assume: 1: hayServ, 2: axeane, 3: cfac, 4: cspv, 5: freelance
    ...
    <section id="journey" className="w-full py-20">
      <div className="py-20 w-full">
        <h1 className="heading">
          {t('heading') || 'Mon'} <span className="text-purple">{t('headingPurple') || 'experience professionnelle'}</span>
        </h1>
        <p className="text-center pt-14 md:tracking-wider mb-4 text-sm md:text-lg lg:text-2xl">
          {t('resumeText') || 'Si vous souhaitez voir plus de détails, comme un CV'}{" "}
          <motion.span
              ...
              <div className="flex lg:flex-row flex-col lg:items-center p-3 py-6 md:p-5 lg:p-10 gap-2">
                ...
                <div className="lg:ms-5">
                  <h1 className="text-start text-xl md:text-2xl font-bold">
                    {t(`${idToKey[card.id]}.title`) || card.title}
                  </h1>
                  <p className="text-start text-white-100 md:text-lg mt-3 font-semibold">
                    {t(`${idToKey[card.id]}.desc`) || card.desc}
                  </p>
                  <p className="text-start text-yellow-600 md:text-base mt-3 font-semibold">
                    {(t(`${idToKey[card.id]}.tasks`) || card.tasks_desc).split(" | ").map((task: string, index: number, array: string[]) => (
